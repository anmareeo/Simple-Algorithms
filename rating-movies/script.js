/**
 * Changed sortMoviesByRank() function to sort movies list by rank
 * Sorted movies by id, rank, and title through dynamic function
 * Created helper function called getMaxMovieObject() for finding max movie
 */
...
*/
window.onload = function() {
let sortedMovies = this.sortMoviesByAttr(movies, 'rank')// choose one of the attributes, which are title, rank, id
 // Display Movies list
displayMovies(sortedMovies);
}
...
function sortMoviesByRank(movies){
  ...
  for (let j = 0; j < movies.length - 1; j++) {
      ...
      for (let i = j; i < movies.length; i++) {
          if (movies[i].rank> max_obj[i].rank) {
              // Know max AND it's index (location)
              //if we find an object with a higher rank, replace max_obj with new object
              max_obj = movies[i]
              max_location = i
          }
      }
